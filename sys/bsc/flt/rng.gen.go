...
func NewRng(min, max Flt) (r Rng) {
	r.Min = min
	r.Max = max
	return r
}
func NewRngArnd(cntr, radius Flt) (r Rng) {
	r.Min = cntr - radius
	r.Max = cntr + radius
	return r
}
...
func (x Rng) Len() Flt         { return x.Max - x.Min }
...
func (x Rng) MinSub(v Flt) Rng {
	x.Min -= v
	return x
}
func (x Rng) MaxAdd(v Flt) Rng {
	x.Max += v
	return x
}
