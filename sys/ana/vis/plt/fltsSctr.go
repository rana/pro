...
func (x *FltsSctr) Measure() vis.Siz { // Plt interface
	if len(x.sctrs) != 0 {
			...
			if !x.Outlier && cndStmSctr.Y.Cnt() > 2 {
				...
				if rngValsMdn != 0 {
					for n := len(*rngVals) - 1; n > -1; n-- { // dsc in case of multiple deletions
						...
						// mu.Unlock()
						// sys.Log("plt.FltsSctr pct:%v", rngVal/rngValsMdn)
						...
						if rngVal/rngValsMdn > OutlierLim { // outlier case observed: 7400%; lim is 1000%
							...
							sys.Logf("plt.FltsSctr %p HIDING OUTLIER POINT %v %v%%", x, del, rngVal.Div(rngValsMdn).Trnc(0))
						}
...
func (x *SctrRndrPnt) Act() {
	vis.RndrRct(x.cntr.X, x.cntr.Y, x.radius, 1, &x.ras)
}
